...
def test_extract_geojson_keep_ids():
    feat_1 = Feature(
        id="custom_id_1",
        geometry=Polygon([[[0, 0], [1, 0], [1, 1], [0, 1], [0, 0]]])
    )
    feat_2 = Feature(
        geometry=Polygon([[[1, 0], [2, 0], [2, 1], [1, 0], [1, 0]]])
    )
    data = FeatureCollection([feat_1, feat_2])
    topo = Extract(data).to_dict()
    objects = topo["objects"]

    assert bool(objects.get("custom_id_1")) is True
    #no id so default feature_{idx} key
    assert bool(objects.get("feature_1")) is True

def test_extract_geojson_keep_ids_catch_duplicates():
    feat_1 = Feature(
        id="dupelicate_id",
        geometry=Polygon([[[0, 0], [1, 0], [1, 1], [0, 1], [0, 0]]])
    )
